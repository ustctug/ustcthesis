...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    示例文档 & \file{main.tex}           & 主文档                       \\
             ...
             & \file{references.bib}     & \BibTeX{} 示例数据库         \\
    \midrule
...
\begin{enumerate}
  \item 在其他平台上配置的 TeX Gyre 系列字体，虽然在风格上比较接近 Times 和 Arial，
    但是与学校官方的 Word 模板字体不完全一致；
  \item Fandol 字库的字形较少，常常出现缺字的情况；
  \item 华文字库和 Fandol 字库虽然不违反学校的要求，
...
\begin{enumerate}
  ...
  \item 与正文平排（\opt{inline}），文献序号作为行文语句的组成部分，
...
\end{latex}
或者使用 \cs{parencite} 在局部序号也可作为行文语句的组成部分。
